...
const CONTRACT_NAME: &str = "crates.io:luna-leverage";
...
pub fn try_increment(deps: DepsMut) -> Result<Response, StdError> {
    STATE.update(deps.storage, |mut state| -> Result<_, StdError> {
        state.count += 1;
        Ok(state)
    })?;
...
pub fn try_reset(deps: DepsMut, info: MessageInfo, count: i32) -> Result<Response, StdError> {
    STATE.update(deps.storage, |mut state| -> Result<_, StdError> {
